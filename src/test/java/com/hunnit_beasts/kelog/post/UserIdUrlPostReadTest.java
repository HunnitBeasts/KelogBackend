...
class UserIdUrlPostReadTest {
    ...
    void setUp() {
        ...
        id = authService.signUp(userDto).getId();
        ...
        postUrl = postService.postCreate(id, postDto).getUrl();
    ...
    @Test
    @DisplayName("로그인한 사용자가 자신의 게시물을 URL로 조회")
    void readOwnPostByUrl() throws Exception {
        mockMvc.perform(get("/posts/{user-id}/{url}", userId, postUrl)
    ...
    @Test
    @DisplayName("로그인하지 않은 사용자가 게시물을 URL로 조회")
    void readPostByUrlAsGuest() throws Exception {
        mockMvc.perform(get("/posts/{user-id}/{url}", userId, postUrl)
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.isFollow").value(false))
    ...
    @Test
    @DisplayName("다른 사용자의 게시물을 팔로우하고 좋아요 누른 후 URL로 조회")
    void readPostByUrlWithFollowAndLike() throws Exception {
        // 새로운 사용자 생성
        ...
        PostLikeRequestDTO postLikeDto = PostLikeRequestDTO.builder()
                .postId(postService.getPostId(userId, postUrl))
                .build();
    ...
    @Test
    @DisplayName("존재하지 않는 게시물 URL 조회")
    void readNonExistentPostByUrl() throws Exception {
        mockMvc.perform(get("/posts/{user-id}/{url}", userId, "non-existent-url")
                        .header("Authorization", token)
                        .accept(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("비공개 게시물을 다른 사용자가 URL로 조회")
    void readPrivatePostByUrlAsOtherUser() throws Exception {
        // 비공개 게시물 생성
        ...
        String privatePostUrl = postService.postCreate(id, privatePostDto).getUrl();
        ...
        mockMvc.perform(get("/posts/{user-id}/{url}", userId, privatePostUrl)
